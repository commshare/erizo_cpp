...
void AMQPHelper::addCallback(const AMQPCallback &cb)
{
    std::unique_lock<std::mutex> lock(callback_queue_mux_);
...
int AMQPHelper::init(const std::string &exchange, const std::string &binding_key, std::function<void(const std::string &msg)> func)
{
    ...
    run_ = true;
    thread_ = std::unique_ptr<std::thread>(new std::thread([&, exchange, binding_key, func]() {
        while (run_)
        {
            amqp_rpc_reply_t res;
    ...
    callback_thread_ = std::unique_ptr<std::thread>(new std::thread([&]() {
        while (run_)
        {
            std::unique_lock<std::mutex> lock(callback_queue_mux_);
            ...
            {
                AMQPCallback cb = callback_queue_.front();
                callback_queue_.pop();
                callback(cb.exchange, cb.queuename, cb.binding_key, cb.msg);
            }
            else
            {
                callback_cond_.wait(lock);
            }
        }
    }));
